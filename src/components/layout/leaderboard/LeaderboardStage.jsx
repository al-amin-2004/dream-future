          ...
          <div className={"pt-30 block md:hidden"}>
            <div className="h-[11rem] grid grid-cols-3 grid-rows-6">
              {reorderedTopThree.map((data) => (
                <div
                  key={data.position}
                  className={`row-end-7 rounded-t-2xl ${data.position === 1 ? 'bg-orange-400 dark:bg-gray-900/90': data.position === 2 ? "bg-orange-400/80 dark:bg-gray-800/50": "bg-orange-400/60 dark:bg-gray-600/50"} p-1`}
                  style={{'grid-row-start': `${data.position}`}}
                >
                  <div
                  ...
                  >
                    <div className="size-8/12 relative">
                      <KingIcon className={`size-12 absolute bottom-full left-1/2 -translate-x-1/2 mb-1 ${data.position === 1 ? "block" : "hidden"}`}/>


                      <div className={`rounded-full overflow-hidden border-2
                        ${data.position === 1 ? "border-orange-400" : data.position === 2 ? "border-sky-500" : "border-green-500"} 
                        ${data.position === 2? "scale-90": data.position === 3 && "scale-75" }`}>
                      <Image
                        src={
                          data.img
                    ...
                    <span className={`flex gap-1 items-center font-medium text-white bg-white/30 px-2 rounded-full
                      ${data.position === 1 ? "dark:text-orange-400" : data.position === 2 ? "dark:text-sky-400" : "dark:text-green-400"}`}>
