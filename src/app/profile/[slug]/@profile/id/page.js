...
const Profile = async ({ params }) => {
    ...
    const anotherUser = user.another[id - 1]
        ...
        <div className="flex flex-col md:flex-row gap-10 md:h-full mt-5 md:mt-2">
            <div className="bg-white w-full h-full rounded-2xl space-y-2 md:space-y-5 flex-2/3">
                <div className="flex justify-center h-[45%]">
                    <Image src={
                        anotherUser.image
                            ? `https://drive.google.com/uc?export=view&id=${anotherUser.image}`
                ...
                <div className="text-black p-3 space-y-6">
                    <h1 className="text-xl md:text-3xl font-heading font-semibold text-center">{anotherUser.name}</h1>

                    <div className="border-b border-border flex justify-between md:px-3">
                        <h2 className="font-medium">Number:</h2>
                        <p className="text-sm md:text-base">{anotherUser.mobile}</p>
                    </div>
                    <div className="border-b border-border flex justify-between md:px-3">
                        <h2 className="font-medium">Email:</h2>
                        <p className="text-sm md:text-base">{anotherUser.email}</p>
                    </div>
                    <div className="border-b border-border flex justify-between md:px-3">
                        <h2 className="font-medium">Unique ID:</h2>
                        <p className="text-sm md:text-base">{anotherUser.uid}</p>
                    </div>
                    <div className="border-b border-border flex justify-between md:px-3">
                        <h2 className="font-medium">Blood:</h2>
                        <p className="text-sm md:text-base">{anotherUser.blood}</p>
                    </div>
                </div>
            </div>
                ...
                <div className="bg-white h-full rounded-xl p-2 md:p-5">
                    <div className="flex flex-col-reverse md:flex-row justify-between items-start gap-2">
                        <div>
                            ...
                            <p className="text-xl md:text-3xl font-medium md:font-bold">{anotherUser.totalMoney}.00 BDT</p>
                        </div>
                    ...
                    <div className="flex justify-between md:justify-around text-center bg-slate-400 py-1.5 md:py-3 px-2.5 rounded-2xl mt-3 md:mt-28">
                        <div className="space-y-1.5">
                            <p className="bg-orange-400 size-11 mx-auto p-1.5 rounded-full font-medium md:text-xl">{anotherUser.totalDeposit}</p>
                            <p className="text-sm md:text-base">Total Diposit</p>
                        </div>
                        <div className="space-y-1.5">
                            <p className="bg-orange-400 size-11 mx-auto p-1.5 rounded-full font-medium md:text-xl">{anotherUser.totalExtra}</p>
